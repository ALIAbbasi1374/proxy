      ...
      <div class="sidebar">
        ...
        <ul class="sidebar-menu" data-widget="tree">
          ...
          <li class="active treeview"> <a href="#"> <i class="fa fa-table"></i> <span>Tables</span> <span class="pull-right-container"> <i class="fa fa-angle-left pull-right"></i> </span> </a>
            <ul class="treeview-menu">
              <li class="active"><a href="table-basic.php"><i class="fa fa-angle-left"></i> Basic Tables</a></li>
        ...
        <div class="content">
            <div class="card">
                <div class="card-body">
                    ...
                    <button  onclick="loadModal('new', '','', 0, 0, 0, 0)" class="btn btn-success fl-l">افزودن به فروشگاه</button>
...
<div class="modal fade" id="myModalEdit" tabindex="-1" role="dialog" aria-labelledby="itemHanderLabel" aria-hidden="true">
      ...
      <div class="modal-body">
        <form>
          ...
          <div class="form-group">
            <label for="shop-name" class="col-form-label">عنوان:</label>
            <input type="text" class="form-control" id="shop-name">
          </div>
          <div class="form-group">
                  <label for="shop-type" class="col-form-label">نوع:</label>
                  <select class="form-control" id="shop-type">
                    <option value="life" selected>جون</option>
                    <option value="ticket" selected>بلیط</option>
                    <option value="golden" selected>طلایی</option>
                  </select>
          <div class="form-group">
            <label for="shop-quantity" class="col-form-label">تعداد:</label>
            <input class="form-control" id="shop-quantity" type="number">
          </div>
          <div class="form-group">
            <label for="shop-price" class="col-form-label">مبلغ پرداختی:</label>
            <input class="form-control" id="shop-price" type="number">
          </div>
          <div class="form-group">
            <label for="shop-price" class="col-form-label">مبلغ پرداختی الکی:</label>
            <input class="form-control" id="shop-fullPrice" type="number">
          </div>
          <div class="form-group">
                  <input type="checkbox" data-width="100" data-height="40" id="active" data-toggle="toggle" data-on="فعال" data-off="غیر فعال" data-onstyle="success" data-offstyle="danger" checked> مخفی کردن
          </div>
        </form>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-primary" id="shop-saveBtn">ذخیره</button>
      </div>
    </div>
<!-- ./wrapper -->
...
<script>
  $.ajax({
    type: "GET",
    url: "https://panel.qday-app.com/panel/shopList",
    
    headers: {
      "Authorization": "Basic ZWhzYW46ZWhzYW4"
    },
    success: function (response) {
      $.each(response.life, function (i, e) {
        var q = `
          <tr id="row" role="row" class="odd">
              <th scope="row">${e.id}</th>
              <td id="name">${e.name}</td>
              <td id="shop-type">${e.type}</td>
              <td id="quantity">${e.quantity}</td>
              <td id="price">${e.price}</td>
              <td id="fullPrice">${e.fullPrice}</td>
              <td>
                  <button type="button" class="btn btn-primary btn-sm" onclick="loadModal(${e.id},'${e.name}','${e.type}', ${e.quantity}, ${e.price}, ${e.fullPrice}, 0)"><i class="fa fa-pencil"></i></button>
              </td>
              <td data-order="0">
                  <button onclick="toggleItem(this,1,'16')" class="btn btn-danger btn-sm"><i
  ...
  function loadModal(id, title,type, quantity, price, fullPrice, active){
    let modal = $('#myModalEdit')
    $(modal).find('#shop-newID').val(id)
    $(modal).find('#shop-name').val(title)
    $(modal).find('#shop-type').val(type)
    $(modal).find('#shop-quantity').val(quantity)
    $(modal).find('#shop-price').val(price)
    $(modal).find('#shop-fullPrice').val(fullPrice)
    $(modal).find('#active').val(active)
    $(modal).modal('show')
    
  }
  
  $('#shop-saveBtn').click(function (e) { 
    e.preventDefault();
    let url = 'https://panel.qday-app.com/panel/';
    if($('#shop-newID').val()=='new'){
      url += 'shopAdd';
    }else{
      url += 'shopListEdit';
    }
    $.ajax({
        ...
        data: JSON.stringify({
          name: $('.modal #shop-name').val(),
          type:$('.modal #shop-type').val(),
          quantity: $('.modal #shop-quantity').val(),
          price: $('.modal #shop-price').val(),
          fullPrice: $('.modal #shop-fullPrice').val(),
          isVisible: 0
        }),
        headers: {
        "Authorization": "Basic ZWhzYW46ZWhzYW4"
